...
%
% DESCRIPTION:
%   This script simulates a lap of a closed circuit track (eg. endurance)
% 
% KNOW ISSUES:
%
% CHANGE LOG:
...
function Output = LapSim_closedTrack(Car, TrackData)
    ...
    % Find apexes
    % [~,apexIndices] = findpeaks((-maxSpeed));
    % Track.apex = zeros(length(Track.d),1);
    % for n = apexIndices
    %     Output.speed(n) = maxSpeed(n);
    %     Track.apex (n) = 1;
    % end
    ...
    %% Prepare input and outputs for simulation
    startIndex = find(Output.speed == min(nonzeros(Output.speed)), 1 );
    % Perform circular shift to bring start point to beggining of the array
    Output.time = circshift(Output.time,-(startIndex-1));
    Output.dTime = circshift(Output.dTime,-(startIndex-1));
    Output.speed = circshift(Output.speed,-(startIndex-1));
    Output.accelX = circshift(Output.accelX,-(startIndex-1));
    Output.throttleOrBrake = circshift(Output.throttleOrBrake,-(startIndex-1));
    Output.stateVar = circshift(Output.stateVar,-(startIndex-1));
    Track.d = circshift(Track.d,-(startIndex-1));
    Track.X = circshift(Track.X,-(startIndex-1));
    Track.Y = circshift(Track.Y,-(startIndex-1));
    Track.curvature = circshift(Track.curvature,-(startIndex-1));
    Track.apex = circshift(Track.apex,-(startIndex-1));
    Track.unfilteredCurvature = circshift(Track.unfilteredCurvature,-(startIndex-1));
    Track.dDistance = circshift(Track.dDistance,-(startIndex-1));
    maxSpeed = circshift(maxSpeed,-(startIndex-1));
    ...
    while true
        ...
        for s=1:numSector
            ...
            if repeat == false
                ...
                waitbar(0.96, progressBar, strcat('Resimulating sector',32, num2str(s),32, 'out of',32, num2str(numSector)));
            end

            if s<numSector
                sectorEndIndex = (sectorIndex(s+1)-1);
                nextSpectorStartSpeed = Output.speed(sectorEndIndex+1);
            else
                sectorEndIndex = length(Track.d);
                nextSpectorStartSpeed = Output.speed(1);
            end
            ...
            for n=sectorStartIndex:sectorEndIndex         
                ...
                if ~(n==length(Track.d))
                    ...
                    if ~(Track.apex(n+1)==1)
                        ...
                        Output.dTime(n+1) = (-Output.speed(n)+sqrt(Output.speed(n)^2+4*0.5*Output.accelX(n)*(Track.dDistance(n+1))))/Output.accelX(n); %quadratic solution to d=0.5*at^2+vt
                        Output.speed(n+1) = sqrt(Output.speed(n)^2+2*Output.accelX(n)*Track.dDistance(n+1));
                    end
                else
                    %Calculate time to get from last point to the first point
                    Output.dTime(1) = (-Output.speed(n)+sqrt(Output.speed(n)^2+4*0.5*Output.accelX(n)*(Track.dDistance(1))))/Output.accelX(n); %quadratic solution to d=0.5*at^2+vt
                end
            end
            ...
            while n>sectorStartIndex
                ...
                if isnan(Output.speed(prev)) || Output.speed(prev)>Output.speed(curr)
                    ...
                    if isnan(Output.accelX(prev))
                        ...
                        if n<sectorEndIndex+1 && ~(n==1)
                            % if not at the end of sector redo the previous point.
                            % Do this by adding 2 to i so that at next step it is
                            % i+1.
                            n=n+2;
                        elseif n==1
                            %if speed at beggining of first sector is changed, the
                            %first sector needs to be resimulated
                            repeat = true;
                        end
                    end
                ...
                n=n-1;
                if repeat == true
                    if ~(sectorEndIndex == length(Track.d))
                        if Output.speed(sectorEndIndex+1)==nextSectorStartSpeed
                            repeat = false;
                            break
                        end
                    end
    ...
    load('2016FSUKEnduranceLapData.mat')
    plot(DistanceData,SpeedData/3.6)
    title('Lap Time Simulation Comparison Curves')
    legend('Simulation', 'Track Data')
    xlabel('Distance (m)')
    ylabel('Speed (m/s)')
    beep
end